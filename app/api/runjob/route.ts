import { AMOUNT_UPDATED, EMAIL_ID_PROVIDE_ERROR, JOB_RUNNED, MONGO_DB_ERROR, NO_USER_FOUND_ERROR, SET_MONTH_AMOUNT_ERROR } from "@/app/errors/errorMessages";
        ...
        try {
            ...
            if (userData) {
                if (userData?.monthlyLimitAmount) {
                    if (parseInt(userData?.todayDate) !== getTodayDate()) {
                        await user.updateOne({ emailId }, {
                            ...
                            $set: {
                                ...
                                totalMonthSpend: userData?.daySpend + userData?.totalMonthSpend
